...
- `redact_credentials()` recognizes credential families in plaintext and
  ...
  forge tokens, package-registry tokens, Coder agent/session and Kiro API-key
  key-value forms, and database connection URIs carrying embedded credentials.
  Key-value matching is JSON-aware and value classes are
