...
- **Resume guard:** `session/load` (resume) is only attempted when Tool Search is disabled AND the session host confirms the prior transcript. Local hosting checks its local Kiro session path (`~/.kiro/sessions/cli/<sid>.json`); Coder hosting probes the validated id in the workspace and sends the derived remote path. A stale persisted sid falls back to `session/new`.
