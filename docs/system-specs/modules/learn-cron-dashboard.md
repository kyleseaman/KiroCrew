...
Cron scheduling, due scans, admission, retries, persistence, and delivery remain
gateway-owned when an admitted parent session executes in Coder. That remote
parent and its descendants may call `kirocrew-cron` through the same
session-scoped stdio proxy as other managed MCP servers, so mutations still run
...
fixed `GET /api/mcp/oauth/callback` route is authenticated by one-shot OAuth
state and bypasses only the dashboard cookie gate. DNS-rebinding Host validation
remains active, and the redirect URI is built at startup from configured HTTPS,
validated Tailscale, or exact loopback state rather than request headers. A
successful authenticated MCP response marks the banner complete; a bounded
OAuth failure marks it failed without reflecting provider details.
