...
- **Coder tokens** — `CODER_AGENT_TOKEN` and `CODER_SESSION_TOKEN` key-value forms
  are redacted in shell-style and JSON environment dumps. Their values are opaque
  and have no dependable prefix, so the bounded key name is the credential anchor.
