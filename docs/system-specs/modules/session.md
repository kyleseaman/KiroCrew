...
Managed Coder sessions override the ordinary local idle timeout with the
...
dedicated descendant. It clones a Coder host's workspace, remote cwd, transport
selection, and gateway-only transport credentials so the local Coder CLI can
open another SSH channel, but never clones its capability registry or
per-session relay bearers into the workspace.
