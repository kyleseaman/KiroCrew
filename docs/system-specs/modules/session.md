...
| `environment_registry()` | Returns the provider-neutral registry owned by the current default provider factory. Its public catalog contains only provider ids, display metadata, and selectable configurations; credentials and protected resource identity stay inside adapters. The gateway lifecycle reconciler resolves it on every pass so a Settings refresh takes effect without restarting the service. |
| `coder_workspace_manager()` | Compatibility projection of the Coder adapter's protected manager, or `None` while managed Coder placement is disabled. New common lifecycle and selection code uses `environment_registry()`. |
...
Managed placement is excluded from ordinary speculative eager spawn. A provider
may positively opt into focus prefetch only for an exact existing session whose
durable binding says its compute is already running. Coder then re-verifies the
workspace through its control plane and uses a no-start host; a missing, stopped,
starting, replaced, or unallocated workspace refuses prefetch without allocating
or starting compute. The first real turn remains the only path allowed to create
or wake billable compute. A successfully prefetched remote runtime uses the same
bounded unclaimed-resume TTL as local focus prefetch, while the 15-minute managed
runtime warm window remains unchanged. A slot persists its selected profile, and
the first real turn passes it to the provider factory. Allocation records the
resolved template and preset in the
...
copy. The stage events remain available to the active transcript and diagnostics, while
the session list collapses them into its ordinary turn-activity label.
...
Closing or archiving a dashboard slot with a managed environment binding is an explicit
...
dedicated descendant. It clones any `RemoteSessionHost`; the Coder host clone
keeps its workspace, remote cwd, transport selection, and gateway-only transport
credentials so the local Coder CLI can open another SSH channel, but never clones
its capability registry or per-session relay bearers into the workspace.
