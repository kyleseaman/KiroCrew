...
class RunEventCoordinator(ManagerComponent):
    ...
    async def _run_inner_impl(
        ...
        def _pin_dedicated_host() -> None:
            """Keep a dedicated child on its PARENT's remote host.

            A subagent taking the dedicated-process arm would otherwise resolve
            its own location from the session default, which splits a family
            across hosts: the parent's turns run in a managed remote workspace
            while its child runs locally against a different filesystem. Only a
            REMOTE parent host is pinned -- a local parent needs no override, and
            the resolver is skipped when it is async so this stays non-blocking.
            """
            resolver = getattr(self._manager._sessions, "subagent_session_host", None)
