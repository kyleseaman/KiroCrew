...
class CleanupDeps:
    ...
    #: The shared stateless background session key. Reclaiming it must not fire
    #: the consolidation callback -- see ``_expire_idle``.
    get_background_key: Callable[[], str]
    ...
    async def _expire_idle(self, timeout_secs: int) -> None:
        ...
        async with self._owner._lock:
            for key, session in self._owner._sessions.items():
                ...
                if session.semaphore.locked():
                    continue
                # A session hosted in a MANAGED REMOTE environment carries its own
                # warm window: the host, not the gateway's global idle timeout,
                # decides how long its runtime is worth keeping. That is the
                # branch's actual remote-reclamation lever -- a remote runtime
                # left warm for the global 30 minutes is billed compute nobody is
                # using. A negative/absent value means the host expresses no
                # opinion and the global timeout applies as before.
                ...
                if session_timeout <= 0 and managed_warm is None:
                    continue
                idle = now - session.last_used > session_timeout
                orphaned = (
        ...
        for key, is_orphan in expired:
            ...
            # The background session is STATELESS and shared: it is reclaimable on
            # this branch (not in ``_PERSISTENT_KEYS``) precisely because nothing
            # about it is worth keeping. Consolidating its transcript on every
            # reclamation would spend an LLM pass on a scratch session no user
            # owns, so the expiry callback is skipped for it alone.
            if self._owner.on_session_expire and key not in self._deps.get_persistent_keys() | {
                self._deps.get_background_key()
            }:
                try:
