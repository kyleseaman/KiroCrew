...
def _confirm_destination(profile: str, region: str, account: str, bucket: str | None) -> bool:
    ...
    if not token.is_file():
        print(
            ...
            "   That directory is a keystone path — the agent cannot create the file "
            "for you, which is the entire point: a terminal check would not do, "
            "because a pty is something any process can allocate.\n"
    ...
    try:
        approved = json.loads(token.read_text() or "{}")
    ...
    # Consume the authorization so it cannot be replayed into a later redirection.
    try:
        token.unlink()
    except OSError as e:
        print(f"❌ Could not consume the authorization file ({e}); refusing rather than "
              f"leaving a reusable one behind.")
        _audit_setup("denied", f"authorization not consumable: {e}")
        return False
    _audit_setup(
        "completed",
