...
_CREDENTIAL_PATTERNS = re.compile(
    ...
    # Coder injects its live agent token into workspace commands even when the
    # agent authenticates with cloud instance identity, and the gateway holds a
    # Coder API bearer plus its own API key. Their values are opaque with no
    # stable prefix, so the bounded environment KEY NAMES are the only usable
    # credential anchors. Prefilter anchors: "CODER_" + "KIRO_API_KEY".
...
# stands in for.
#
# THE MAINTENANCE HAZARD this is built against: adding a 25th branch to
...
_CREDENTIAL_PREFILTER_LITERALS: tuple[str, ...] = (
    ...
    "aws_access_key_id",
    "SessionToken",
    # Session-host credential env names (CODER_AGENT_TOKEN, CODER_SESSION_TOKEN,
    # KIRO_API_KEY). Two anchors for one branch: "CODER_" covers both Coder
    # spellings, and the Kiro key shares no substring with them.
    "CODER_",
    "KIRO_API_KEY",
...
def redact_credentials(text: str) -> tuple[str, list[str]]:
    ...
    # This is the hot path — the alternation is 24 branches retried at nearly
