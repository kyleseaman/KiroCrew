...
_CREW_HIDDEN_LEAVES: tuple[str, ...] = (
    ...
    *(f"{AUTH_SQLITE_DB}{suffix}" for suffix in AUTH_SQLITE_SIDECAR_SUFFIXES),
    # The Coder workspace ownership registry, its lock, and the quarantined copy a
    # corrupt-parse rename leaves behind. Each record binds a session to a workspace
    # UUID that stop/delete act on, so a writable file could redirect a destructive
    # Coder call at a workspace the gateway never owned, and the quarantined original
    # carries the same records verbatim. HIDDEN rather than READONLY because nothing
    # inside the sandbox opens it: both readers -- the CLI's workspace commands and
    # the ACP session host -- run gateway-side, so masking costs no live consumer.
    # An empty registry denies every binding, which is the safe direction; unlike a
    # governance ceiling, hiding this does not resolve to a permissive default.
    "coder_workspaces.json",
    "coder_workspaces.lock",
    "coder_workspaces.json.corrupt",
)
