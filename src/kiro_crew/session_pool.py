...
class WarmSessionPool:
    ...
    async def start_pool(self, *, blocking: bool = True) -> None:
        ...
        # A gateway whose interactive sessions run in a MANAGED REMOTE
        # environment leaves its local background session lazy: remote work
        # should not pay for one always-resident local kiro-cli process before a
        # background task actually needs one. The session is still created on
        # demand by ``_ensure_background`` at first use, so nothing is lost --
        # only the eager boot-time spawn is skipped.
        registry = self._owner.environment_registry()
        ...
        if not managed_environments:
            ...
            self._deps.logger.info(
                "Managed environment configured; local background session is lazy"
            )
