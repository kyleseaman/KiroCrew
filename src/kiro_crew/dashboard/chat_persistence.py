...
def _apply_recent_session(
    ...
    *,
    ...
    slot._disk_meta_observed = bool(meta)
    slot._memory_assignment_from_history = True
    # Managed session environment. The durable binding wins; the older
    # coder_* keys are the compatibility projection a transcript written
    # before the binding existed still carries.
    _environment = SessionEnvironmentBinding.from_dict(meta.get("environment"))
    if _environment is not None:
        slot.environment = _environment
    else:
        if isinstance(meta.get("coder_profile"), str):
            slot.coder_profile = meta["coder_profile"]
        if isinstance(meta.get("coder_workspace"), str):
