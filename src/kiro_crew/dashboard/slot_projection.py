    ...
    def to_dict(
        ...
        *,
        ...
        return {
            ...
            # Managed session environment: the durable public binding plus the
            # compatibility projections a pre-environment frontend still reads.
            # Provider-owned secrets and immutable lifecycle records stay out of
            # dashboard state, so only non-secret identity travels here.
            "environment": (slot.environment.to_dict() if slot.environment is not None else None),
            "coder_profile": slot.coder_profile,
            "coder_workspace": slot.coder_workspace,
