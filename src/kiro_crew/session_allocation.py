        ...
        if pooled is not None:
            ...
            async with self._start_sem:
                try:
                    ...
                    # Track the STARTING provider so surfaces can show a remote
                    # environment booting. It is deliberately absent from
                    # ``_sessions`` until the handshake succeeds, so without this
                    # a managed session reports no execution location for the
                    # whole cold start. Untracked on both exits below.
                    owner._track_starting_provider(key, provider)
                    try:
                        await provider.start()
                    except BaseException:
                        owner._untrack_starting_provider(key, provider)
                        raise
                except (asyncio.CancelledError, Exception):
                    if preparation.revision:
