...
#: Bounded re-resolve attempts for the provider-backed background handle. An attempt
#: loses only when the idle sweep popped the entry between the ensure and the
#: semaphore acquire; a small bound absorbs that interleaving, while staying a bound
#: (rather than ``while True``) keeps a pathological reset loop from spinning here.
_BG_ACQUIRE_MAX_ATTEMPTS = 3
...
class _BackgroundSessionEntry(Protocol):
    ...
    prompt_count: int
    #: Monotonic idle stamp read by ``session_cleanup._expire_idle``. The
    #: background entry is not in ``_PERSISTENT_KEYS`` on this branch, so the
    #: acquisition path below refreshes it — see ``_provider_backed_bg_session``.
    last_used: float
...
class _ProviderBgSession:
    ...
    def adopt_semaphore(self) -> None:
        """Record that the CALLER already acquired this entry's turn semaphore.

        ``_provider_backed_bg_session`` acquires it before handing the handle out,
        so the handle owns it for its whole lifetime rather than only during a
        turn. That is what makes the idle sweep's ``skip_if_busy`` re-check see
        this entry as busy, so it cannot be popped while a live handle still
        points at its provider. ``destroy`` releases it.
        """
        self._sem_held = True
    ...
    async def prompt(
        ...
        #
        # Release only what THIS call acquired. When the handle already owns the
        # semaphore (the acquisition path took it so the sweep would refuse to
        # reap the entry), releasing here would hand the entry back mid-lifetime
        # and reopen the window the ownership exists to close -- ``destroy`` is
        # what releases in that case.
        acquired_here = False
...
class BackgroundSessionRuntime:
    ...
    async def _provider_backed_bg_session(self) -> object:
        ...
        # Take OWNERSHIP of the entry before handing a handle out, then re-validate
        # it under the owner lock. Both halves are load-bearing:
        #
        #   * ``BACKGROUND_KEY`` is not in ``_PERSISTENT_KEYS`` on this branch (a
        #     gateway whose interactive sessions run in a managed remote environment
        #     must be able to reclaim its local background runtime), so the idle
        #     sweep CAN reap this entry.
        #   * ``session_cleanup._expire_idle`` selects candidates in one locked pass
        #     and resets them in a LATER, unlocked one. Its second pass re-checks
        #     ONLY ``semaphore.locked()`` (via ``reset(skip_if_busy=True)``, under
        #     the owner lock) -- never ``last_used``. So a freshened timestamp does
        #     NOT save an already-selected candidate; holding the semaphore does,
        #     and it is the same primitive the sweep already treats as "busy".
        #   * ``reset`` POPS the entry and shuts its provider down rather than
        #     respawning in place, and the handle reads ``self._sess.provider``
        #     lazily at stream time, so a handle over a popped entry would stream on
        #     a dead provider. Re-validating identity while already holding the
        #     semaphore closes that: past this point the sweep cannot pop it.
        #
        # A lost race (the entry was popped between the ensure and the acquire) is
        # retried rather than surfaced -- ``_ensure_background`` recreates on demand,
        # which is what makes reclamation transparent to callers.
        key = self._deps.background_key
        for _ in range(_BG_ACQUIRE_MAX_ATTEMPTS):
            await self._owner._ensure_background()
            async with self._owner._lock:
                sess = self._owner._sessions.get(key)
            if sess is None:
                continue
            await sess.semaphore.acquire()
            # From here the permit is HELD BUT NOT YET TRANSFERRED to a handle.
            # Every escape from this region must release it: a cancellation while
            # awaiting the owner lock, or a raising factory/adopt, would otherwise
            # leave the entry permanently ``semaphore.locked()`` -- which the sweep
            # reads as busy forever, so the entry can never be reclaimed AND every
            # later background caller blocks on it. ``transferred`` is flipped only
            # once the handle owns the permit and will release it in ``destroy``.
            transferred = False
            try:
                async with self._owner._lock:
                    still_registered = self._owner._sessions.get(key) is sess
                if not still_registered:
                    # Selected and reset while we waited: re-resolve rather than
                    # hand back a handle over a dead provider. Released in the
                    # finally below, then retried.
                    continue
                # Idle accounting, NOT the safety mechanism: without it
                # ``last_used`` is only ever set at creation, so a
                # continuously-used background session would be reaped a timeout
                # later and pay a cold start on next use. Safety is the held
                # semaphore above.
                sess.last_used = time.monotonic()
                handle = self._deps.provider_bg_session_factory(sess)
                # The factory contract REQUIRES ownership transfer. A handle that
                # cannot adopt the permit would acquire it again in ``prompt`` and
                # deadlock on this very permit, so an unexpected factory is a
                # programming error surfaced here rather than a hang later.
                adopt = getattr(handle, "adopt_semaphore", None)
                if not callable(adopt):
                    raise TypeError(
                        "provider_bg_session_factory must return a handle exposing "
                        "adopt_semaphore(); got "
