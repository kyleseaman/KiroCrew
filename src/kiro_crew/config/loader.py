...
def _build_session_config(session_data: dict) -> SessionConfig:
    return SessionConfig(
        ...
        coder=_build_coder_session_config(session_data),
    )


def _build_coder_session_config(session_data: dict) -> _sections.CoderSessionConfig:
    """Build ``session.coder`` from the raw section.

    ``enabled`` is TRISTATE and must stay so: only a real JSON boolean is
    authoritative, and anything else (absent, null, a hand-edited string) resolves
    to ``None`` -- the compatibility sentinel that lets the legacy environment
    variables still apply. Coercing it to a bool here would silently turn "never
    configured" into "explicitly off" and strand an installation that has not
    saved the Settings panel yet.
    """
    coder_data = session_data.get("coder", {})
    if not isinstance(coder_data, dict):
        coder_data = {}
    raw_enabled = coder_data.get("enabled")
    # Defaults are read off the dataclass rather than imported, the same way
    # _build_orchestrator_config reads max_plan_duration_seconds: this module's
    # top-level names are a FROZEN compatibility facade
    # (test_loader_reexports_historical_snapshot_by_identity), so a new re-export
    # may not be added here.
    _defaults = _sections.CoderSessionConfig
    return _sections.CoderSessionConfig(
        enabled=raw_enabled if isinstance(raw_enabled, bool) else None,
        url=str(coder_data.get("url", "")),
        template=str(coder_data.get("template", "")),
        preset=str(coder_data.get("preset", "")),
        profiles=_sections._coerce_coder_profiles(coder_data.get("profiles")),
        remote_cwd=str(coder_data.get("remote_cwd", _defaults.remote_cwd)),
        runtime_warm_minutes=_safe_int(
            coder_data.get("runtime_warm_minutes", _defaults.runtime_warm_minutes),
            _defaults.runtime_warm_minutes,
        ),
        stop_after_minutes=_safe_int(
            coder_data.get("stop_after_minutes", _defaults.stop_after_minutes),
            _defaults.stop_after_minutes,
        ),
        delete_after_days=_safe_int(
            coder_data.get("delete_after_days", _defaults.delete_after_days),
            _defaults.delete_after_days,
        ),
        max_running=_safe_int(
            coder_data.get("max_running", _defaults.max_running),
            _defaults.max_running,
        ),
        workspace_prefix=str(coder_data.get("workspace_prefix", _defaults.workspace_prefix)),
    )
