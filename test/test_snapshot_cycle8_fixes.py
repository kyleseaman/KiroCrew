...
class TestReplaceSavesEveryTreeBeforeReplacingAny:
    def test_the_rollback_copy_is_taken_before_any_database_is_swapped(self):
        ...
        src = inspect.getsource(snap._do_replace)
        save_at = src.index("_copytree_safe(d, backup / tree")
        swap_at = src.index("_backup_and_copy(mc, backup, snap, comp)")
        replace_at = src.index("_clear_tree_root(d)")
        assert save_at < swap_at, (
            "the memory trees are saved after a database was already replaced"
        )
