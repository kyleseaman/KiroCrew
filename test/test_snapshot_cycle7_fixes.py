...
class TestSetupPreservesWhatItDidNotWrite:
    ...
    def test_a_kms_encryption_config_is_not_downgraded_to_aes256(self, home, monkeypatch):
        """The shared hardening step applies AES256 unconditionally. On a bucket the
        operator configured with a customer-managed key, that is a downgrade — and it
        would pass the privacy read-back, because any SSE counts as encrypted."""
        # The key-id field is deliberately omitted. The code under test re-puts the
        # whole configuration verbatim and never reads that field, so the fixture does
        # not need it — and its AWS field name trips the inclusive-language gate, which
        # scans a diff of added lines through stdin and therefore cannot honour an
        # inline suppression comment. Please do not "complete" this fixture.
        ...
        assert puts, "encryption was never applied"
        last = puts[-1]
        sent = json.loads(last[last.index("--server-side-encryption-configuration") + 1])
        algo = sent["Rules"][0]["ApplyServerSideEncryptionByDefault"]["SSEAlgorithm"]
        assert algo == "aws:kms", "a customer-managed key was replaced with AES256"
