...
class TestAFailedHardeningPutsTheEncryptionKeyBack:
    """Hardening is several API calls and the encryption one lands FIRST.

    A failure after that point leaves the bucket on AES256 with the operator's KMS
    default already gone — and the next attempt reads AES256 as the "prior" config, so
    the original becomes unrecoverable. The window is small; the loss is permanent.
    """

    def test_the_kms_config_is_restored_when_hardening_raises(self, home, monkeypatch):
        fake = _fake()
        calls: list[str] = []

        def failing_harden(bucket, profile, tagset):
            calls.append("hardened")
            raise remote.DestinationError("tagging refused")

        monkeypatch.setattr(engine, "run_aws", fake)
        monkeypatch.setattr(engine, "harden_bucket", failing_harden)
        with pytest.raises(remote.DestinationError) as e:
            remote.setup_destination("p", "us-west-2")
        assert "tagging refused" in str(e.value), "the original error was masked"
        assert calls == ["hardened"]

        puts = fake.argv_for("s3api put-bucket-encryption")
        assert puts, "the KMS configuration was not put back"
        sent = json.loads(puts[-1][puts[-1].index(
            "--server-side-encryption-configuration") + 1])
        algo = sent["Rules"][0]["ApplyServerSideEncryptionByDefault"]["SSEAlgorithm"]
        assert algo == "aws:kms"

    def test_a_restore_failure_does_not_mask_the_real_error(
        self, home, monkeypatch, capsys
    ):
        """The message that explains what went wrong must survive."""
        answers = {"s3api head-bucket": (0, "", "")}
        fake = _fake(answers)
        fake.answers["s3api put-bucket-encryption"] = (255, "", "AccessDenied")
        ...
        with pytest.raises(remote.DestinationError) as e:
            remote.setup_destination("p", "us-west-2")
        assert "tagging refused" in str(e.value)
        assert "Could not put the bucket's KMS encryption" in capsys.readouterr().out

    def test_a_non_kms_bucket_needs_no_restore(self, home, monkeypatch):
        fake = _fake()
