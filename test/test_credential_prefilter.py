...
BRANCH_SAMPLES: tuple[str, ...] = (
    ...
    "CODER_AGENT_TOKEN=notarealcodertokenvalue123",  # 4  session-host credentials
    "-----BEGIN RSA PRIVATE KEY-----\nMIIBOgIBAAJBAKj34\n-----END RSA PRIVATE KEY-----",  # 5
    "xoxb-1234567890-abcdefghijklmnop",  # 6  Slack
    f"123456789:{_A35}",  # 7  Telegram bot token
    "M" + "a" * 24 + ".abc123." + "b" * 27,  # 8  Discord bot token
    "ghp_" + "a" * 36,  # 9  GitHub PAT
    "github_pat_" + "a" * 40,  # 10
    "glpat-" + "a" * 20,  # 11
    "sk_live_" + "a" * 24,  # 12  Stripe
    "SG.abcdefghijklmnop.abcdefghijklmnop",  # 13  SendGrid
    "sk-proj-" + "a" * 20,  # 14  OpenAI
    "sk-ant-" + "a" * 20,  # 15  Anthropic
    "npm_" + "a" * 30,  # 16
    "pypi-" + "a" * 20,  # 17
    "dop_v1_" + "a" * 45,  # 18  DigitalOcean
    "GOCSPX-" + "a" * 25,  # 19  Google OAuth client secret
    "postgres://dbuser:s3cr3tpw@db.example.com:5432/app",  # 20  URI userinfo
    "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxIn0.abcdefghijklmnop",  # 21  JWS
    f" eyJ{_LINK_PAYLOAD}.{_LINK_SIG}",  # 22  2-segment dashboard link token
    "Authorization: Bearer abc.def.ghijklmnop",  # 23  HTTP/JSON bearer
)
...
# alone and expands U+0130 to two code points. Branch 23 is `(?i:Authorization)`,
...
def test_every_pattern_branch_has_a_prefilter_anchor() -> None:
    ...
    Adding a 25th branch to `_CREDENTIAL_PATTERNS` without an anchor in
...
def test_differential_test_detects_a_dropped_pattern_branch(
    ...
    crippled = _rebuild([b for i, b in enumerate(BRANCHES) if i != 9])  # drop gh[opsur]_
...
def test_widening_a_branch_cannot_outgrow_its_anchor() -> None:
    ...
    pre-fix implementation via branch 23 (`Authorization: Bearer`), whose
