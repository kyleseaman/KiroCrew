...
in-memory state, so each stop removes the ephemeral node and each later start
rejoins with a fresh node identity instead of accumulating offline devices. Do
not bind automation to a session node's Tailscale IP. Coder's workspace agent
authenticates with AWS instance identity, so the template stores no static Coder
agent token. Configure the separate gateway-only Coder API bearer in **Settings
→ Session Environments** after the workspace is reachable.
