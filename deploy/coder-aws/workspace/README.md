...
This dogfood template has a separate POC credential boundary: it places the Kiro API
key under `coder` ownership and exports it to `kiro-cli`. The workspace user can read
the shared credential. Base64 is transport encoding, not encryption, and
every workspace configured with the same SSM parameter receives the same key. Rotate
the key regularly and after a suspected workspace compromise. A production deployment
should issue a short-lived credential per workspace or broker model authentication
through a trusted gateway instead of sharing a long-lived key.
